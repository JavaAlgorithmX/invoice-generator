...
export interface InvoiceItem {
  id: string;
  productName: string;
  company: string; // Replaced breed
  qty: number;
  sku: number; // Replaced mfr (Unit Size in KG)
  ratePerKg: number; // Replaced mrp & rate
  exp: string;
  dis: number;
  gst: number;
  amount: number;
...
const styles = StyleSheet.create({
  ...
  // Updated Column Widths to match 100% total
        ...
        <View style={styles.table}>
          ...
          {data.items.map((item, index) => (
            <View key={item.id} style={styles.tableRow}>
              ...
              <Text style={styles.colQty || 0}>{item.qty}</Text>
              <Text style={styles.colSku || 0}>{item.sku}</Text>
              <Text style={styles.colRate || 0}>{item.ratePerKg.toFixed(2)}</Text>
              <Text style={styles.colExp}>{item.exp}</Text>
              <Text style={styles.colDis|| 0}>{item.dis}</Text>
              <Text style={styles.colGst|| 0}>{item.gst}</Text>
