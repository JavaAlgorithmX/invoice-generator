...
//     vendorGSTIN: '10FAKPS8863N1ZV',
//     vendorPhone: '+91 9771064575',
//     bankName: 'YESBANK',
//     branch: 'SARAIYAGANJ',
//     accountNo: '121261900003022',
//     ifsc: 'YESB0001212',
//     upiId: 'yespay mabs1212619ikit3022@yesbankltd',
//     items: [
//       { id: '1', productName: '', company: '', qty: 1, sku: 0, ratePerKg: 0, exp: '', dis: 0, gst: 0, amount: 0 },
//     ],
...
//       }));
//     }
//   }, [data.items, data.discount, data.gstAmount, data.grandTotal, data.subTotal]);

//   const resetCustomer = () => {
//     if (confirm("Clear all Customer Details?")) {
//       setData({ ...data, customerName: '', customerAddress: '', customerPhone: '', customerGSTIN: '', shipToName: '', shipToAddress: '' });
//     }
//   };

//   const resetInvoice = () => {
//     if (confirm("Reset Invoice Details?")) {
//       setData({ ...data, invoiceNo: '', date: getTodayDateString() });
//     }
//   };

//   const resetItems = () => {
//     if (confirm("Clear all Products?")) {
//       setData({ ...data, items: [{ id: Date.now().toString(), productName: '', company: '', qty: 1, sku: 0, ratePerKg: 0, exp: '', dis: 0, gst: 0, amount: 0 }] });
//     }
//   };
...
//   const isCustomerNameFilled = data.customerName.trim().length > 0;
//   const safeCustomerName = data.customerName.trim().replace(/[^a-zA-Z0-9-]/g, '_').replace(/_+/g, '_');
  
//   let formattedFileDate = data.date;
//   if (data.date) {
//     const [year, month, day] = data.date.split('-');
//     if (year && month && day) formattedFileDate = `${day}-${month}-${year}`;
//   }
  
//   const generatedFileName = `${safeCustomerName}_${formattedFileDate}.pdf`;
...
//              <button onClick={resetCustomer} className="bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded shadow transition text-sm">
//                Reset Customer
//              </button>
//              <button onClick={resetInvoice} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow transition text-sm">
//                Reset Invoice Info
//              </button>
//              <button onClick={resetItems} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded shadow transition text-sm">
//                Clear Products
//              </button>
//            </div>
//         </div>
...
//           <div className="md:col-span-2 p-6 bg-slate-100 border-4 border-slate-300 rounded-xl">
//             <h2 className="text-lg font-bold text-slate-800 uppercase mb-4 tracking-wider">Customer Details</h2>
//             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
//               <div>
//                 <label className="block text-sm font-bold text-slate-700 mb-1">
//                   Customer Name (Billed To) <span className="text-red-600 text-lg leading-none">*</span>
//                 </label>
//                 <input type="text" name="customerName" required value={data.customerName} onChange={handleGeneralChange} className={`${highVisInput} border-red-300 focus:border-red-500`} placeholder="Enter Customer Name" />
//               </div>
//               <div><label className="block text-sm font-bold text-slate-700 mb-1">Mobile No</label><input type="text" name="customerPhone" value={data.customerPhone} onChange={handleGeneralChange} className={highVisInput} placeholder="Enter Mobile Number" /></div>
//               <div className="md:col-span-2"><label className="block text-sm font-bold text-slate-700 mb-1">Address</label><input type="text" name="customerAddress" value={data.customerAddress} onChange={handleGeneralChange} className={highVisInput} placeholder="Enter Full Address" /></div>
//               <div><label className="block text-sm font-bold text-slate-700 mb-1">GSTIN (Optional)</label><input type="text" name="customerGSTIN" value={data.customerGSTIN} onChange={handleGeneralChange} className={highVisInput} placeholder="Enter GSTIN if available" /></div>
//             </div>

//             <h3 className="text-md font-bold text-slate-700 mb-4 border-b-2 border-slate-200 pb-2 uppercase tracking-wider">Shipping Details (Ship To)</h3>
//             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//               <div><label className="block text-sm font-bold text-slate-700 mb-1">Ship To Name</label><input type="text" name="shipToName" value={data.shipToName} onChange={handleGeneralChange} className={highVisInput} placeholder="Leave blank if same as Billed To" /></div>
//               <div><label className="block text-sm font-bold text-slate-700 mb-1">Ship To Address</label><input type="text" name="shipToAddress" value={data.shipToAddress} onChange={handleGeneralChange} className={highVisInput} placeholder="Leave blank if same as Billed To" /></div>
//             </div>
//           </div>
          
//           <div className="p-6 bg-slate-100 border-4 border-slate-300 rounded-xl">
//             <h2 className="text-lg font-bold text-slate-800 uppercase mb-4 tracking-wider">Invoice Info</h2>
//             <div className="flex flex-col gap-6">
//               <div><label className="block text-sm font-bold text-slate-700 mb-1">Invoice No</label><input type="text" name="invoiceNo" value={data.invoiceNo} onChange={handleGeneralChange} className={highVisInput} placeholder="e.g. OFC000087" /></div>
//               <div><label className="block text-sm font-bold text-slate-700 mb-1">Invoice Date</label><input type="date" name="date" value={data.date} onChange={handleGeneralChange} className={`${highVisInput} cursor-pointer`} /></div>
//             </div>
//           </div>
//         </div>
...
//                 <th className="p-2 border-r border-slate-600 w-32 text-center">Exp Date</th>
...
//                   <td className="p-1 border-r border-slate-300"><input type="text" value={item.productName} onChange={(e) => handleItemChange(index, 'productName', e.target.value)} className={highVisTableInput} placeholder="Product Name" /></td>
//                   <td className="p-1 border-r border-slate-300"><input type="text" value={item.company} onChange={(e) => handleItemChange(index, 'company', e.target.value)} className={highVisTableInput} placeholder="Company" /></td>
...
//                   <td className="p-1 border-r border-slate-300"><input type="date" value={item.exp} onChange={(e) => handleItemChange(index, 'exp', e.target.value)} className={`${highVisTableInput} text-center cursor-pointer`} /></td>
...
//         <div className="flex flex-col items-center border-t-4 border-slate-200 pt-10">
//           {isCustomerNameFilled ? (
//             <PDFDownloadLink document={<InvoicePDF data={data} />} fileName={generatedFileName} className="bg-blue-700 text-white px-10 py-5 rounded-2xl font-black text-xl hover:bg-blue-800 hover:shadow-2xl transition-all border-4 border-blue-900 transform hover:-translate-y-1">
//               {({ loading }) => (loading ? 'Generating Document...' : 'DOWNLOAD OFFICIAL PDF INVOICE')}
//             </PDFDownloadLink>
//           ) : (
//             <div className="text-center">
//               <button disabled className="bg-slate-400 text-white px-10 py-5 rounded-2xl font-black text-xl border-4 border-slate-500 cursor-not-allowed opacity-70">
//                 DOWNLOAD OFFICIAL PDF INVOICE
//               </button>
//               <p className="text-red-600 font-bold mt-4 text-lg">⚠️ Please enter a Customer Name to unlock downloading.</p>
//             </div>
//           )}
//         </div>
//       </div>
...
export default function InvoiceGenerator() {
  const [data, setData] = useState<InvoiceData>({
    ...
    // LOCKED DEFAULT VENDOR DETAILS
    vendorName: 'CLOVERFIELD CREAMERY & ORGANICS',
    vendorDesc: 'FROM NATURE TO NUTRITION', 
    vendorTagline: '',
    vendorAddress: 'Raghopur, Near Karikh Asthan Salempur, Muzaffarpur Bihar PIN 842004',
    vendorEmail: 'ccorganicss@gmail.com',
    vendorGSTIN: '10FAKPS8863N1ZV',
    vendorPhone: '+91 9771064575',
    
    // LOCKED DEFAULT BANK DETAILS
    beneficiaryName: 'CLOVERFIELD CREAMERY AND ORGANICS',
    bankName: 'YESBANK',
    branch: 'SARAIYAGANJ',
    accountNo: '121261900003022',
    ifsc: 'YESB0001212',
    upiId: 'yespay.mabs1212619ikit3022@yesbankitd',
    ...
    <div className="max-w-screen-2xl mx-auto p-4 md:p-8 bg-slate-200 min-h-screen">
      <div className="bg-white p-6 rounded-xl shadow-2xl border-2 border-slate-300">
        ...
        <div className="flex flex-col md:flex-row gap-8 mb-8">
          <div className="flex-1 p-6 bg-green-50 border-4 border-green-200 rounded-xl grid grid-cols-1 md:grid-cols-2 gap-4">
            <h3 className="col-span-1 md:col-span-2 font-black text-green-900 text-lg uppercase border-b-2 border-green-200 pb-2">Bank Details</h3>
            <div className="md:col-span-2"><label className="block text-xs font-bold text-green-800 mb-1 uppercase">Beneficiary Name</label><input type="text" name="beneficiaryName" value={data.beneficiaryName} onChange={handleGeneralChange} className={highVisInput} /></div>
            <div><label className="block text-xs font-bold text-green-800 mb-1 uppercase">Bank Name</label><input type="text" name="bankName" value={data.bankName} onChange={handleGeneralChange} className={highVisInput} /></div>
            <div><label className="block text-xs font-bold text-green-800 mb-1 uppercase">Branch</label><input type="text" name="branch" value={data.branch} onChange={handleGeneralChange} className={highVisInput} /></div>
            <div><label className="block text-xs font-bold text-green-800 mb-1 uppercase">Account No</label><input type="text" name="accountNo" value={data.accountNo} onChange={handleGeneralChange} className={highVisInput} /></div>
            <div><label className="block text-xs font-bold text-green-800 mb-1 uppercase">IFSC Code</label><input type="text" name="ifsc" value={data.ifsc} onChange={handleGeneralChange} className={highVisInput} /></div>
